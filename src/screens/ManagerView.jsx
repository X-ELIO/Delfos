                    ...
                    {objs.map((obj, i) => (
                      <div key={obj.id ?? i} style={s.objRow}>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                          <span style={{ ...s.typeBadge, color: TYPE_COLOR[obj.type] ?? 'var(--ac)' }}>
                            {obj.type}
                          </span>
                          {obj.by_when && <span style={{ fontSize: 11, color: 'var(--tx2)' }}>{obj.by_when}</span>}
                          {obj.score != null && (
                            <span style={{ fontSize: 11, color: 'var(--tx2)', fontFamily: 'var(--font-mono)', marginLeft: 'auto' }}>
                              {obj.score}%
                            </span>
                          )}
                        </div>
                        <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx)', marginBottom: obj.description ? 4 : 0 }}>
                          {obj.title}
                        </p>
                        {obj.description && (
                          <p style={{ fontSize: 12, color: 'var(--tx2)', lineHeight: 1.5 }}>{obj.description}</p>
                        )}
                        {obj.feedback && (
