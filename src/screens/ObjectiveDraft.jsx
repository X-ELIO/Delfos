...
function detectPeopleKpiViolation(objectives) {
  return objectives.filter(o => {
    if (o.source === 'delfos') return false
...
// ── Shared badge styles ────────────────────────────────────────────────────
const badge = {
  obj:     { background: 'var(--card-2)', color: 'var(--tx2)', fontSize: 10, fontWeight: 700,
             letterSpacing: '0.08em', padding: '3px 8px', borderRadius: 4 },
  delfos:  { background: 'var(--ac)', color: '#fff', fontSize: 10, fontWeight: 700,
             letterSpacing: '0.06em', padding: '3px 8px', borderRadius: 4 },
  perf:    { background: 'rgba(16,185,129,0.12)', color: 'var(--ok)', fontSize: 10, fontWeight: 700,
             letterSpacing: '0.06em', padding: '3px 8px', borderRadius: 4, border: '1px solid rgba(16,185,129,0.3)' },
  team:    { background: 'var(--purple)', color: '#fff', fontSize: 10, fontWeight: 700,
             letterSpacing: '0.06em', padding: '3px 8px', borderRadius: 4 },
  learn:   { background: 'var(--blue)', color: '#fff', fontSize: 10, fontWeight: 700,
             letterSpacing: '0.06em', padding: '3px 8px', borderRadius: 4 },
  thresh:  { background: 'rgba(239,68,68,0.12)', color: 'var(--err)', fontSize: 10, fontWeight: 700,
             letterSpacing: '0.06em', padding: '3px 8px', borderRadius: 4, border: '1px solid rgba(239,68,68,0.4)' },
}

function TypeBadge({ type }) {
  if (type === 'performance') return <span style={badge.perf}>PERFORMANCE</span>
  if (type === 'team')        return <span style={badge.team}>TEAM</span>
  if (type === 'learning')    return <span style={badge.learn}>LEARNING</span>
  return null
}
...
export default function ObjectiveDraft({ onNavigate, onSettings, onEmployeeView, onManagerView, onCoverageView, activeTab, onLogout }) {
  ...
  const [editingObj,       setEditingObj]       = useState({})   // { [id]: { title, description } }
  ...
  const anyScored     = objectives.some(o => o.score != null)
  const active        = objectives.filter(o => o.status !== 'ignored')
  const avg           = active.length
    ? Math.round(active.reduce((s, o) => s + (o.score ?? 0), 0) / active.length)
    : 0
  const portfolioColor = scoreColor(avg, thresh)
  const statusLabel    = avg >= thresh.green ? 'GREEN' : avg >= thresh.min ? 'AMBER' : 'RED'

  // ── Objective CRUD helpers ────────────────────────────────────────────────
  function addObjective() {
    setObjectives(p => [
      ...p,
      { id: Date.now(), type: 'performance', title: '', description: '', source: 'user', status: 'active' },
    ])
  }
  ...
  // ── Ask Delfos (global suggestions) ──────────────────────────────────────
  async function handleAskDelfos() {
    setLoading(true); startTimer('asking')
    try {
  ...
  async function handleScore() {
    const toScore = objectives.filter(o => o.status !== 'ignored' && o.title.trim())
    if (!toScore.length) return
    setLoading(true); startTimer('scoring')
    try {
      const result  = await scoreObjectives({ profile, objectives: toScore, cascade })
      stopTimer()
      ...
      // Merge back — keep unscored (ignored / empty) objectives as-is
      setObjectives(prev => prev.map(o => {
        const updated = scored.find(s => s.id === o.id)
        return updated ? { ...o, ...updated } : o
      }))
      setPortfolioSummary(summary)
      // Clear any open edit sessions after re-score
      setEditingObj({})
    } catch (err) {
      stopTimer()
      console.error('score-objectives error:', err)
      setError('Error scoring objectives. Please try again.')
    } finally {
      setLoading(false)
    }
  }

  // ── Per-card: Ask Delfos to improve ──────────────────────────────────────
  ...
  async function acceptProposal(obj) {
    ...
    setObjectives(p => p.map(o => o.id === obj.id ? { ...o, ...improved, source: 'delfos', was_improved: true } : o))
    ...
    try {
      ...
      if (rescored?.[0]) {
        setObjectives(p => p.map(o => o.id === obj.id ? { ...o, ...rescored[0] } : o))
      }
    } catch (err) {
  ...
  function startEdit(obj) {
    setEditingObj(p => ({ ...p, [obj.id]: { title: obj.title, description: obj.description ?? '' } }))
  }
  ...
  async function saveEdit(obj) {
    const ed         = editingObj[obj.id]
    const updatedObj = { ...obj, title: ed.title, description: ed.description }
    setObjectives(p => p.map(o => o.id === obj.id ? { ...o, title: ed.title, description: ed.description, source: 'user' } : o))
    cancelEdit(obj.id)
  ...
  async function handleSubmit() {
    setSaving(true)
    try {
      const activeObjs  = objectives.filter(o => o.status !== 'ignored')
      const ignoredObjs = objectives.filter(o => o.status === 'ignored')
      ...
      const avgScore    = activeObjs.length
        ? Math.round(activeObjs.reduce((s, o) => s + (o.score ?? 0), 0) / activeObjs.length)
        : 0
      ...
      const { data: sub, error: subErr } = await supabase
        ...
        .insert({
          ...
          objectives_ignored:  ignoredObjs.length,
        })
        .select('id')
        .single()
      ...
      const objRows = activeObjs.map((obj, i) => ({
      ...
      setError(`Error saving: ${err?.message ?? String(err)}`)
    } finally {
      setSaving(false)
    }
  }
  ...
  if (loading) return (
    <LoadingScreen
      action={loadCtx.action}
      elapsed={loadCtx.elapsed}
  ...
  return (
    <Shell step={1} onSettings={onSettings} onEmployeeView={onEmployeeView} onManagerView={onManagerView} onCoverageView={onCoverageView} activeTab={activeTab} onLogout={onLogout}>
      <div style={ds.page}>
        ...
        <h1 style={ds.heading}>Set &amp; Score Your Objectives</h1>

        {/* ── Portfolio summary — only when any objective has a score ── */}
        {anyScored && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: 11, color: 'var(--tx2)', marginLeft: 'auto' }}>claude-haiku-4-5-20251001</span>
            </div>
            ...
            <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                ...
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6,
                              border: `1px solid ${portfolioColor}`, borderRadius: 6, padding: '3px 10px' }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: portfolioColor, display: 'inline-block' }} />
                  <span style={{ fontSize: 11, fontWeight: 700, background: portfolioColor, color: '#000',
                                 borderRadius: 4, padding: '1px 6px' }}>{statusLabel}</span>
                </div>
              </div>
              {portfolioSummary && (
        ...
        {/* Team objective required banner */}
        ...
        {kpiViolations.length > 0 && (
          <div style={ds.kpiViolationBanner}>
            <p style={{ fontWeight: 700, marginBottom: 4 }}>⚠ People KPI conflict — remove before scoring</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {objectives.map((obj, i) => {
            const isDraft   = obj.score == null || editingObj[obj.id] != null
            const ed        = editingObj[obj.id]
            ...
            const isImproving = !!improving[obj.id]

            // ── DRAFT CARD ──────────────────────────────────────────────
            if (isDraft) {
              const titleVal = ed ? ed.title : obj.title
              const descVal  = ed ? ed.description : (obj.description ?? '')
                ...
                <div key={obj.id} style={ds.objCard}>
                  ...
                  <div style={ds.objCardHead}>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      {obj.source === 'delfos' && <span style={badge.delfos}>DELFOS</span>}
                    </div>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      {(['performance', 'learning', 'team']).map(t => (
                        <button key={t}
                          disabled={!!ed}
                          onClick={() => !ed && update(obj.id, 'type', t)}
                          style={{
                            ...ds.typeBtn,
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <div>
                      ...
                      <input style={ds.input}
                        value={titleVal}
                        onChange={e => ed
                          ? setEditingObj(p => ({ ...p, [obj.id]: { ...p[obj.id], title: e.target.value } }))
                          : update(obj.id, 'title', e.target.value)
                        }
                    ...
                    <div>
                      ...
                      <textarea style={{ ...ds.input, minHeight: 60, resize: 'vertical' }}
                        value={descVal}
                        onChange={e => ed
                          ? setEditingObj(p => ({ ...p, [obj.id]: { ...p[obj.id], description: e.target.value } }))
                          : update(obj.id, 'description', e.target.value)
                        }
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                      <button
                        style={{ background: 'var(--ok)', color: '#fff', border: 'none',
                                 ...
                                 padding: '6px 14px', cursor: isImproving ? 'default' : 'pointer',
            ...
            return (
              <div key={obj.id} style={{
                background: 'var(--card)',
                border: `1px solid ${belowMin && obj.status !== 'ignored' ? 'rgba(239,68,68,0.5)' : 'var(--border)'}`,
                borderRadius: 12,
                padding: '16px 18px',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', flex: 1 }}>
                    <span style={badge.obj}>OBJ {i + 1}</span>
                    {obj.source === 'delfos' && <span style={badge.delfos}>DELFOS</span>}
                    <TypeBadge type={obj.type} />
                    {belowMin && obj.status !== 'ignored' && <span style={badge.thresh}>BELOW THRESHOLD</span>}
                    {obj.weight > 0 && (
                ...
                {obj.status !== 'ignored' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8,
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      <button
                        style={{ background: 'var(--ai-soft)', border: '1px solid var(--ai-border)',
                                 color: 'var(--purple)', fontSize: 12, padding: '5px 12px',
                                 borderRadius: 6, cursor: isImproving ? 'default' : 'pointer',
                                 opacity: isImproving ? 0.6 : 1 }}
                        disabled={isImproving}
                      ...
                      <button
                        style={{ background: 'none', border: 'none', color: 'var(--err)',
                                 fontSize: 12, padding: '5px 12px', borderRadius: 6, cursor: 'pointer' }}
                        onClick={() => update(obj.id, 'status', 'ignored')}>
                        Ignore
                      </button>
                    </div>
                    ...
                    {proposals[obj.id] && (
                      <div style={{ background: 'var(--ai-soft)', border: '1px solid var(--ai-border)',
                        ...
                        <button
                          style={{ marginTop: 10, background: 'var(--ok)', color: '#fff', border: 'none',
                                   borderRadius: 6, fontSize: 12, fontWeight: 600, padding: '6px 14px', cursor: 'pointer' }}
                          onClick={() => acceptProposal(obj)}>
        ...
        {/* ── Footer ── */}
        <div style={ds.footer}>
          {/* Left: type pills + Ask Delfos */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {(['performance', 'learning', 'team']).map(t => {
              const disabled = t === 'team' && ['C', 'D'].includes(profile?.archetype_code)
              return (
                <button key={t}
                  disabled={disabled}
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {(() => {
              ...
              return (
                <button
                  style={{ ...ds.scoreBtn, opacity: canScore ? 1 : 0.4 }}
                  disabled={!canScore}
                  title={
                  ...
                  onClick={handleScore}>
                  {anyScored ? 'Re-score →' : 'Score →'}
                </button>
              )
            })()}
            {anyScored && (
              <button
                style={{ background: 'var(--ok)', color: '#fff', border: 'none', borderRadius: 8,
                         fontSize: 14, fontWeight: 600, padding: '9px 20px', cursor: saving ? 'default' : 'pointer',
...
// ── Draft page styles ──────────────────────────────────────────────────────
const ds = {
  page:              { maxWidth: 700, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 },
  ...
  objNumBadge:       { background: 'var(--card-2)', color: 'var(--tx2)', fontSize: 10, fontWeight: 700,
                       letterSpacing: '0.08em', padding: '3px 8px', borderRadius: 4 },
  typeBtn:           { fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 5, border: 'none' },
